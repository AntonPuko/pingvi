...
{
    public  class GtoRangeParser : IDisposable {

        private const char Offsuited = 'o';
        private const char Suited = 's';

        private List<GtoRangeHand> _parsedHands;

        

        public GtoRangeParser() {
            _parsedHands = new List<GtoRangeHand>();
        }
       
        public  List<GtoRangeHand> Parse(string handNames) {
        ...
        private void ParseOffsuited(string name)
        {
            ...
            for (int i = 1; i <= 4; i++) {
                for (int j = 1; j <= 4; j++) {
                    ...
                    _parsedHands.Add(new GtoRangeHand(c1, c2));
                }
            }
